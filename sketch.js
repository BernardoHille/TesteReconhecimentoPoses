...
// helper para p5.js v2 (sem preload)
...
async function setup() {
  ...
  // carrega a moeda (sprite)
  try {
    coinImg = await loadImageAsync('coin.webp'); // ajuste o caminho se necessário
  } catch (e) {
...
function draw() {
  ...
  // Atualiza e desenha (agora como moeda)
  for (let ball of balls) {
    ...
    if (coinImg) {
      // desenha a moeda centralizada na posição da "bolinha"
      push();
      imageMode(CENTER);
      image(coinImg, ball.x, ball.y, COIN_SIZE, COIN_SIZE);
      pop();
    } else {
      // fallback visual se a imagem ainda não carregou
      fill(0, 100, 255);
      noStroke();
  ...
  // Colisão com esqueleto e keypoints (inalterado)
...
function createBall() {
  return {
    x: random(width),
    y: random(-height, 0),
    r: BALL_RADIUS // colisão permanece consistente
  };
}
